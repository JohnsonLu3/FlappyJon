...
class Game extends Phaser.State {
	...
	init() {
		...
		this.game.add.tween(this.game.world).to({
			alpha: 1
		}, 200, Phaser.Easing.Default.In, true);
	...
	onTap() {
		if (this.ready.visible) {
			...
			this.timer = this.time.events.loop(2000, () => new Pipe(this.game, this.pipes));
		}
	...
	onRestart() {
		...
		let add = this.game.add;
		let tween = this.game.add.tween(this.game.world).to({
			alpha: 0
		}, 100, Phaser.Easing.Default.In, true);
